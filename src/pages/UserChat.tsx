...
export default function UserChat() {
  ...
  const [longPressImageId, setLongPressImageId] = useState<string | null>(null);
...
${conversationText}`;
  ...
  // Bug5: 表情包长按删除
  const handleImageLongPressStart = (imgId: string) => {
    setLongPressImageId(imgId);
    if (longPressTimerRef.current) {
      clearTimeout(longPressTimerRef.current);
    }
    longPressTimerRef.current = setTimeout(() => {
      const img = imageLibrary.find(i => i.id === imgId);
      if (img && confirm(`确定要删除表情「${img.name}」吗？`)) {
        deleteImageFromLibrary(imgId);
        setImageLibrary(prev => prev.filter(i => i.id !== imgId));
        showToast('已删除表情', 'info', '🗑️');
      }
      setLongPressImageId(null);
    }, 800); // 800ms长按
  };
  ...
  // Bug10: 保存桌面快捷方式（下载.url文件）
  const handleSaveShortcut = () => {
    const urlContent = `[InternetShortcut]\nURL=https://2insd402nc4c.space.minimaxi.com/`;
    const blob = new Blob([urlContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'AI聊天平台.url';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // ============ 表情包触发逻辑 ============
      // 用户上传表情（命名+URL），AI读取表情库，首次提及100%触发，后续随机概率
      const emojiImages = getImageLibrary();
      if (emojiImages.length > 0 && userConfig.aiCanSendImages !== false) {
        // 检查AI回复中是否提到了表情名称
        const triggeredImage = emojiImages.find(img =>
          img.name && response.content.includes(img.name)
        );

        if (triggeredImage) {
          // 检查是否是首次提及（检查最近10条消息中是否有使用过这个表情）
          const recentMsgs = getMessages(currentCharacter.id).slice(-10);
          const isFirstMention = !recentMsgs.some(m =>
            m.imageUrl === triggeredImage.url &&
            m.role === 'assistant'
          );

          // 首次提及100%触发，后续有30%概率
          if (isFirstMention || Math.random() < 0.3) {
            // 延迟1秒发送表情（模拟AI主动发送）
            setTimeout(() => {
              const emojiMsg = saveMessage({
                role: 'assistant',
                content: `[${currentCharacter?.name}发送了表情：${triggeredImage.name}]`,
                imageUrl: triggeredImage.url,
                characterId: currentCharacter!.id,
                userId: 'current_user',
              });
              setMessages(prev => [...prev, emojiMsg]);
        ...
        <div className="max-w-4xl mx-auto px-4 py-4 space-y-4">
          ...
          {messages.map((msg, msgIndex) => (
            ...
            >
              ...
              {/* 消息操作按钮 */}
              <div className={`absolute ${msg.role === 'user' ? 'left-0' : 'right-0'} top-1/2 -translate-y-1/2 ${msg.role === 'user' ? '-translate-x-2' : 'translate-x-2'} flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity`}>
                {/* 删除按钮（所有消息） */}
                <button
                  onClick={() => {
                    if (confirm('确定删除这条消息？')) {
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1" style={{ maxHeight: 'calc(90vh - 140px)' }}>
              ...
              <div className="border-t pt-4">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-lg hover:from-blue-600 hover:to-purple-600 transition-colors"
                >
                  <Download className="w-4 h-4" />
                  <span className="text-sm font-medium">保存桌面快捷方式</span>
                </button>
                <p className="text-xs text-gray-500 mt-1">
                  下载 .url 文件到本地，双击即可打开网站
                </p>
              </div>
              ...
              <div className="border-t pt-4">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 text-red-600 border border-red-200 rounded-lg hover:bg-red-100 transition-colors"
                >
                  <Trash2 className="w-4 h-4" />
                  <span className="text-sm font-medium">清空所有对话记录</span>
                </button>
                <p className="text-xs text-red-500 mt-1">
                  ⚠️ 此操作将清空所有角色的所有对话，且无法恢复
                </p>
              </div>
            </div>
                ...
                {(imageSearch ? searchImages(imageSearch) : imageLibrary).map((img) => (
                  <div
                    ...
                    className={`relative aspect-square rounded-lg overflow-visible group ${
                      longPressImageId === img.id ? 'ring-2 ring-red-400' : ''
                    }`}
                  >
                    <button
                      onClick={() => handleSelectImage(img)}
                      // Bug5: 长按删除
                      onMouseDown={() => handleImageLongPressStart(img.id)}
                      onMouseUp={handleImageLongPressEnd}
                      onMouseLeave={handleImageLongPressEnd}
                      onTouchStart={() => handleImageLongPressStart(img.id)}
                      onTouchEnd={handleImageLongPressEnd}
                      onTouchCancel={handleImageLongPressEnd}
