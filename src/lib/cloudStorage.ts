...
// 用户调教数据类型
export interface UserTuningData {
  customPrompt: string;
  forceMemory: string;
  updatedAt: string;
}
...
// ============ 用户调教设置（用户个人数据，按用户ID+角色ID隔离）============

// 生成用户调教数据存储键
function getUserTuningKey(characterId: string): string {
  const userId = getCurrentUserId();
  return `user_tuning_${userId}_${characterId}`;
}

// 保存用户调教设置到云端
export async function saveUserTuningToCloud(
  characterId: string,
  tuning: UserTuningData
): Promise<boolean> {
  try {
    if (!characterId) return false;
    const key = getUserTuningKey(characterId);
    const { error } = await supabase
      .from('cloud_data')
      .upsert({
        id: key,
    ...
    console.error('保存用户调教设置到云端失败:', error);
    return false;
  }
}

// 从云端获取用户调教设置
export async function getUserTuningFromCloud(
  characterId: string
): Promise<UserTuningData | null> {
  try {
    ...
    if (error || !data) return null;
    return data.data_json as UserTuningData;
  } catch (error) {
    console.error('获取用户调教设置失败:', error);
    return null;
  }
}

// 删除用户调教设置（云端）
export async function deleteUserTuningFromCloud(characterId: string): Promise<boolean> {
  try {
    if (!characterId) return false;
    const key = getUserTuningKey(characterId);
    const { error } = await supabase
    ...
    console.error('删除用户调教设置失败:', error);
    return false;
  }
}
